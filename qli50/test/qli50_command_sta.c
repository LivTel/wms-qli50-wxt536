/* qli50_command_sta.c
** Open a connection to the Vaisala Qli50, and send a STA command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Qli50 Sta: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Sta","qli50_command_sta.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	fprintf(stdout,"Sending STA to Qli50.\n");
	if(!Wms_Qli50_Command_Sta("Qli50 Sta","qli50_command_sta.c",reply_string,255))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Sta:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Sta:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Sta:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Qli50 Sta:Help.\n");
	fprintf(stdout,"Qli50 Sta sends a STA command to the Vaisala Qli50.\n");
	fprintf(stdout,"qli50_command_sta [-serial_device|-se <filename>]\n");

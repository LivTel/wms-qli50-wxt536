/* qli50_command_close.c
** Open a connection to the Vaisala Qli50, and send a close command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Qli50 Close: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Close","qli50_command_close.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	fprintf(stdout,"Sending CLOSE to Qli50.\n");
	if(!Wms_Qli50_Command_Close("Qli50 Close","qli50_command_close.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"CLOSE sent to Qli50.\n");
	/* close interface */
	if(!Wms_Qli50_Connection_Close("Qli50 Close","qli50_command_close.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Close:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Close:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Close:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Qli50 Close:Help.\n");
	fprintf(stdout,"Qli50 Close closes the Vaisala Qli50.\n");
	fprintf(stdout,"qli50_command_close [-serial_device|-se <filename>]\n");

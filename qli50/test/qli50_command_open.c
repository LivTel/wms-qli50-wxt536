/* qli50_command_open.c
** Open a connection to the Vaisala Qli50, and send an open command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * A character representing the QLI50 Identifier to OPEN. Usually 'A'.
 */
char Qli50_Id = 'A';
 ...
 * @see ../cdocs/wms_qli50_command.html#Wms_Qli50_Command_Open
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Qli50 Open: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Open","qli50_command_open.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	fprintf(stdout,"Sending 'OPEN %c' to Qli50.\n",Qli50_Id);
	if(!Wms_Qli50_Command_Open("Qli50 Open","qli50_command_open.c",Qli50_Id))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 open has been sent.\n");
	/* close interface */
	if(!Wms_Qli50_Connection_Close("Qli50 Open","qli50_command_open.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Open:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Open:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Open:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}

/* qli50_command_echo.c
** Open a connection to the Vaisala Qli50, and send an echo command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * Boolean, used to decide whether to turn the echo on or off.
 */
int Echo_OnOff = FALSE;
 ...
 * @see ../cdocs/wms_qli50_command.html#Wms_Qli50_Command_Echo
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Qli50 Echo: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Echo","qli50_command_echo.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	fprintf(stdout,"Setting Qli50 echo to %d.\n",Echo_OnOff);
	if(!Wms_Qli50_Command_Echo("Qli50 Echo","qli50_command_echo.c",Echo_OnOff))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 echo has been set to %d.\n",Echo_OnOff);
	/* close interface */
	if(!Wms_Qli50_Connection_Close("Qli50 Echo","qli50_command_echo.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Echo:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Echo:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Echo:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Qli50 Echo:Help.\n");
	fprintf(stdout,"Qli50 Echo turns the Vaisala Qli50 echo on or off.\n");
	fprintf(stdout,"qli50_command_echo [-serial_device|-se <filename>][-on|-off]\n");
	fprintf(stdout,"\t[-l[og_level] <number>][-h[elp]]\n");
	fprintf(stdout,"\n");
	fprintf(stdout,"\t-on turns the Qli50 echo on.\n");
	fprintf(stdout,"\t-off turns the Qli50 echo off.\n");

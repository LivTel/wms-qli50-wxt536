/* qli50_command_par.c
** Open a connection to the Vaisala Qli50, and send a PAR command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_qli50_command.html#Wms_Qli50_Command_Par
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Qli50 Par: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Par","qli50_command_par.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	fprintf(stdout,"Sending PAR to Qli50.\n");
	if(!Wms_Qli50_Command_Par("Qli50 Par","qli50_command_par.c",reply_string,255))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"PAR returned %s.\n",reply_string);
	/* close interface */
	if(!Wms_Qli50_Connection_Close("Qli50 Par","qli50_command_par.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Par:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Par:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Par:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Qli50 Par:Help.\n");
	fprintf(stdout,"Qli50 Par sends a PAR command to the Vaisala Qli50.\n");
	fprintf(stdout,"qli50_command_par [-serial_device|-se <filename>]\n");

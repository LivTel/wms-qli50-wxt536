...
** Open a connection to the Vaisala Qli50, and send a read sensors command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
	fprintf(stdout,"Qli50 Read Sensors\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Qli50 Read Sensors: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Qli50_Connection_Open("Qli50 Read Sensors","qli50_command_read_sensors.c",Serial_Device_Name))
	{
		Wms_Qli50_Error();
	...
	if(!Wms_Qli50_Command_Read_Sensors("Qli50 Read Sensors","qli50_command_read_sensors.c",Qli50_Id,Seq_Id))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	/* close interface */
	if(!Wms_Qli50_Connection_Close("Qli50 Read Sensors","qli50_command_read_sensors.c"))
	{
		Wms_Qli50_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Qli50 Read Sensors:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-l")==0)||(strcmp(argv[i],"-log_level")==0))
		{
			if((i+1)<argc)
			{
				...
				if(retval != 1)
				{
					fprintf(stderr,"Qli50 Read Sensors:Parse_Arguments:"
						"Illegal log level %s.\n",argv[i+1]);
					return FALSE;
				}
		...
		{
			fprintf(stderr,"Qli50 Read Sensors:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Qli50 Read Sensors:Help.\n");
	fprintf(stdout,"Qli50 Read Sensors sends an read sensors command to the Qli50.\n");
	fprintf(stdout,"qli50_command_read_sensors [-serial_device|-se <filename>][-qli50_id <id>][-seq_id <id>]\n");
	fprintf(stdout,"\t[-l[og_level] <number>][-h[elp]]\n");
	fprintf(stdout,"\n");
	fprintf(stdout,"\t-qli50_id sets the QLI50 Id parameter to the read sensors command - this should be a single letter.\n");
	fprintf(stdout,"\t-seq_id sets the sequence Id parameter to the read sensors command - this should be a single letter.\n");

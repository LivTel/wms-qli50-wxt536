/* wxt536_command_reset.c
** Open a connection to the Vaisala Wxt536, and send a reset command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * A character identifying the Vaisala Wxt536 device address to be reset.
 */
char Device_Address = ' ';
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Reset
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stdout,"Wxt536 Reset: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Reset","wxt536_command_reset.c",Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Reseting Wxt536 with Device Address '%c'.\n",Device_Address);
	if(!Wms_Wxt536_Command_Reset("Wxt536 Reset","wxt536_command_reset.c",Device_Address))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Wxt536 with Device Address '%c' has been reset.\n",Device_Address);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Reset","wxt536_command_reset.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Wxt536 Reset:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Reset:Parse_Arguments:"
		...
		{
			fprintf(stderr,"Wxt536 Reset:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Reset:Help.\n");
	fprintf(stdout,"Wxt536 Reset resets the Vaisala Wxt536 with the specified device address.\n");
	fprintf(stdout,"wxt536_command_reset [-serial_device|-se <filename>][-d[evice_address] <character>]\n");

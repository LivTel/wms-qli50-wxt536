/* wxt536_command_pressure_temperature_humidity_data_get.c
** Open a connection to the Vaisala Wxt536, retrieve the current pressure/temperature/humidity data and print it out.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wxt536_Command_Pressure_Temperature_Humidity_Data_Struct
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Pressure_Temperature_Humidity_Data_Get
...
{
	struct Wxt536_Command_Pressure_Temperature_Humidity_Data_Struct ptu_data;
		
	fprintf(stdout,"Wxt536 Get Pressure/Temperature/Humidity Data\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Get Pressure/Temperature/Humidity Data: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Get Pressure/Temperature/Humidity Data",
				       "wxt536_command_pressure_temperature_humidity_data_get.c",
				       Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Getting Pressure/Temperature/Humidity Data from Wxt536 with Device Address '%c'.\n",
		Device_Address);
	if(!Wms_Wxt536_Command_Pressure_Temperature_Humidity_Data_Get("Wxt536 Get Pressure/Temperature/Humidity Data",
						 "wxt536_command_pressure_temperature_humidity_data_get.c",
								      Device_Address,&ptu_data))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Air temperature %.2f degrees Centigrade.\n",ptu_data.Air_Temperature);
	fprintf(stdout,"Relative Humidity %.2f %%.\n",ptu_data.Relative_Humidity);
	fprintf(stdout,"Air Pressure %.2f hPA (mbar).\n",ptu_data.Air_Pressure);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Get Pressure/Temperature/Humidity Data",
					"wxt536_command_pressure_temperature_humidity_data_get.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
	...
	fprintf(stdout,"Wxt536 Get Pressure/Temperature/Humidity Data:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Get Pressure/Temperature/Humidity Data:Parse_Arguments:"
		...
		{
			fprintf(stderr,"Wxt536 Get Pressure/Temperature/Humidity Data:"
				"Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Get Pressure/Temperature/Humidity Data:Help.\n");
	fprintf(stdout,"Wxt536 Get Pressure/Temperature/Humidity Data queries the Vaisala Wxt536 and gets the current pressure, temperature and humidity data.\n");
	fprintf(stdout,"wxt536_command_pressure_temperature_humidity_data_get [-serial_device|-se <filename>][-d[evice_address] <character>]\n");

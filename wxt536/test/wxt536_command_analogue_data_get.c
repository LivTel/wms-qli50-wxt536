...
** Open a connection to the Vaisala Wxt536, retrieve the current analogue sensor data and print it out.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wxt536_Command_Analogue_Data_Struct
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Analogue_Data_Get
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Wxt536 Get Analogue Data\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Get Analogue Data: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Get Analogue Data","wxt536_command_analogue_data_get.c",
				       Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Getting Analogue Data from Wxt536 with Device Address '%c'.\n",
		Device_Address);
	if(!Wms_Wxt536_Command_Analogue_Data_Get("Wxt536 Get Analogue Data",
						 "wxt536_command_analogue_data_get.c",
						 Device_Address,&analogue_data))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"PT1000 Temperature %.2f degrees Centigrade.\n",analogue_data.PT1000_Temperaure);
	fprintf(stdout,"Auxiliary Rain Accumulation %.2f mm.\n",analogue_data.Aux_Rain_Accumulation);
	fprintf(stdout,"Ultrasonic Level Voltage %.2f V.\n",analogue_data.Ultrasonic_Level_Voltage);
	fprintf(stdout,"Solar Radiation Voltage %.2f V.\n",analogue_data.Solar_Radiation_Voltage);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Get Analogue Data","wxt536_command_analogue_data_get.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Wxt536 Get Analogue Data:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Get Analogue Data:Parse_Arguments:"
		...
		{
			fprintf(stderr,"Wxt536 Get Analogue Data:"
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Get Analogue Data:Help.\n");
	fprintf(stdout,"Wxt536 Get Analogue Data queries the Vaisala Wxt536 and gets the current analogue sensor data.\n");
	fprintf(stdout,"wxt536_command_analogue_data_get [-serial_device|-se <filename>][-d[evice_address] <character>]\n");

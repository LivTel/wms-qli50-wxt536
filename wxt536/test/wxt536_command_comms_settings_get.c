...
** Open a connection to the Vaisala Wxt536, get the current communication settings, and print them out.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Wxt536 Comms Settings Get\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Comms Settings Get: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Comms Settings Get","wxt536_comms_settings_get.c",
				       Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Wxt536 Comms Settings Get:Finished.\n");
	return 0;
}
...
static int Parse_Arguments(int argc, char *argv[])
{
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			fprintf(stderr,"Wxt536 Comms Settings Get:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}

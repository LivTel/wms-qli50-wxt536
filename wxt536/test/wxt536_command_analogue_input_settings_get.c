/* wxt536_command_analogue_input_settings_get.c
** Open a connection to the Vaisala Wxt536, retrieve the current analogue input settings.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Analogue_Input_Settings_Get
...
{
	double update_interval,averaging_time;
	
	fprintf(stdout,"Wxt536 Analogue Input Settings Get\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Analogue Input Settings Get: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Analogue Input Settings Get",
				       "wxt536_command_analogue_input_settings_get.c",
				       Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Getting Analogue Input Settings from Wxt536 with Device Address '%c'.\n",Device_Address);
	if(!Wms_Wxt536_Command_Analogue_Input_Settings_Get("Wxt536 Analogue Input Settings Get",
							"wxt536_command_analogue_input_settings_get.c",
	...
	fprintf(stdout,"The analogue input update interval is %.3fs.\n",update_interval);
	fprintf(stdout,"The analogue input averaging time is %.3fs.\n",averaging_time);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Analogue Input Settings Get",
					"wxt536_command_analogue_input_settings_get.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
	...
	fprintf(stdout,"Wxt536 Analogue Input Settings Get: Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Analogue Input Settings Get:Parse_Arguments:"
		...
		{
			fprintf(stderr,
				"Wxt536 Analogue Input Settings Get:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Analogue Input Settings Get:Help.\n");
	fprintf(stdout,"This program queries the Vaisala Wxt536 and gets some analogue input settings.\n");
	fprintf(stdout,"wxt536_command_analogue_input_settings_get [-serial_device|-se <filename>][-d[evice_address] <character>]\n");

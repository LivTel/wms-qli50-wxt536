/* wxt536_command_reset_precipitation_counter.c
** Open a connection to the Vaisala Wxt536, and send a reset precipitation counter command.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Reset_Precipitation_Counter
...
{
	fprintf(stdout,"Wxt536 Reset Precipitation Counter\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Reset Precipitation Counter: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Reset Precipitation Counter",
				       "wxt536_command_reset_precipitation_counter.c",Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Reseting preciptation counter of the Wxt536 with Device Address '%c'.\n",Device_Address);
	if(!Wms_Wxt536_Command_Reset_Precipitation_Counter("Wxt536 Reset Precipitation Counter",
							   "wxt536_command_reset_precipitation_counter.c",Device_Address))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Wxt536 with Device Address '%c' has had it's precipitation counter reset.\n",Device_Address);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Reset Precipitation Counter",
					"wxt536_command_reset_precipitation_counter.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
	...
	fprintf(stdout,"Wxt536 Reset Precipitation Counter:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Reset Precipitation Counter:Parse_Arguments:"
		...
		{
			fprintf(stderr,
				"Wxt536 Reset Precipitation Counter:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Reset Precipitation Counter:Help.\n");
	fprintf(stdout,"Wxt536 Reset Precipitation Counter resets the precipitation counter of the Vaisala Wxt536 with the specified device address.\n");
	fprintf(stdout,"wxt536_command_reset_precipitation_counter [-serial_device|-se <filename>][-d[evice_address] <character>]\n");
	...
	fprintf(stdout,"\t-device_address specifies the Wxt536 to reset. This address is normally '0'.\n");

/* wxt536_command_wind_data_set.c
** Open a connection to the Vaisala Wxt536, and set a new solar radiation gain value.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
char Device_Address = ' ';
/**
 * The new solar radiation gain to configure the wxt536 with.
 */
double Gain = 1.0;
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Solar_Radiation_Gain_Set
...
{
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Set\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Set: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Solar Radiation Sensor Gain Set",
	...
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Solar Radiation Sensor Gain Set",
					"wxt536_command_solar_radiation_gain_set.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
	...
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Set:Finished.\n");
	return 0;
}
...
static int Parse_Arguments(int argc, char *argv[])
{
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-g")==0)||(strcmp(argv[i],"-gain")==0))
		{
			if((i+1)<argc)
			{
				retval = sscanf(argv[i+1],"%lf",&Gain);
				if(retval != 1)
				{
					fprintf(stderr,"Wxt536 Solar Radiation Sensor Gain:Parse_Arguments:"
						"Illegal gain value %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
			}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Set:Help.\n");
	fprintf(stdout,"This program set the solar radiation sensor gain parameter in theVaisala Wxt536.\n");
	fprintf(stdout,"wxt536_command_solar_radiation_gain_set [-serial_device|-se <filename>][-d[evice_address] <character>]\n");
	fprintf(stdout,"\t[-g[ain] <number>][-l[og_level] <number>][-h[elp]]\n");
	fprintf(stdout,"\n");
	...
	fprintf(stdout,"\t-gain specifies the gain to use.\n");
}

...
** Open a connection to the Vaisala Wxt536, retrieve the current solar radiation sensor settings and print the gain out.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * Open a connection to the Vaisala Wxt536, retrieve the current solar radiation sensor settings 
 * and print the gain out.
 * @author $Author: cjm $
 */
/* hash definitions */
/**
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Solar_Radiation_Gain_Get
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Get\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain Get: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Solar Radiation Sensor Gain Get",
	...
	fprintf(stdout,"Getting Solar Radiation Sensor Gain from Wxt536 with Device Address '%c'.\n",Device_Address);
	if(!Wms_Wxt536_Command_Solar_Radiation_Gain_Get("Wxt536 Solar Radiation Sensor Gain Get",
							"wxt536_command_solar_radiation_gain_get.c",
							Device_Address,&gain))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"The solar radiation sensor gain is %.2f.\n",gain);
	/* close interface */
	if(!Wms_Wxt536_Connection_Close("Wxt536 Solar Radiation Sensor Gain Get",
					"wxt536_command_solar_radiation_gain_get.c"))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
	...
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain :Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Solar Radiation Sensor Gain:Parse_Arguments:"
		...
		{
			fprintf(stderr,
				"Wxt536 Solar Radiation Sensor Gain:Parse_Arguments:argument '%s' not recognized.\n",
				argv[i]);
			return FALSE;
		}			
	}
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Solar Radiation Sensor Gain:Help.\n");
	fprintf(stdout,"This program queries the Vaisala Wxt536 and gets solar radiation sensor gain setting.\n");
	fprintf(stdout,"wxt536_command_solar_radiation_gain_get [-serial_device|-se <filename>][-d[evice_address] <character>]\n");

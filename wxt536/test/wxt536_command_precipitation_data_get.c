/* wxt536_command_precipitation_data_get.c
** Open a connection to the Vaisala Wxt536, retrieve the current precipitation data and print it out.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * @see ../cdocs/wms_wxt536_command.html#Wxt536_Command_Precipitation_Data_Struct
 * @see ../cdocs/wms_wxt536_command.html#Wms_Wxt536_Command_Precipitation_Data_Get
...
{
	struct Wxt536_Command_Precipitation_Data_Struct precipitation_data;
		
	fprintf(stdout,"Wxt536 Get Precipitation Data\n");
	/* initialise logging */
	...
	{
		fprintf(stdout,"Wxt536 Get Precipitation Data: Specify a serial device filename.\n");
		return 2;
	}
	if(!Wms_Wxt536_Connection_Open("Wxt536 Get Precipitation Data","wxt536_command_precipitation_data_get.c",
				       Serial_Device_Name))
	{
		Wms_Wxt536_Error();
	...
	fprintf(stdout,"Getting Precipitation Data from Wxt536 with Device Address '%c'.\n",
		Device_Address);
	if(!Wms_Wxt536_Command_Precipitation_Data_Get("Wxt536 Get Precipitation Data",
						      "wxt536_command_precipitation_data_get.c",
						      Device_Address,&precipitation_data))
	{
		Wms_Wxt536_Error();
		if(Wms_Serial_Get_Error_Number() != 0)
			Wms_Serial_Error();
	...
	fprintf(stdout,"Rain Accumulation %.2f mm.\n",precipitation_data.Rain_Accumulation);
	fprintf(stdout,"Rain Duration %.2f s.\n",precipitation_data.Rain_Duration);
	fprintf(stdout,"Rain Intensity %.2f mm/h.\n",precipitation_data.Rain_Intensity);
	...
	*/
	...
	fprintf(stdout,"Wxt536 Get Precipitation Data:Finished.\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if((strcmp(argv[i],"-d")==0)||(strcmp(argv[i],"-device_address")==0))
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Wxt536 Get Precipitation Data:Parse_Arguments:"
		...
		{
			fprintf(stderr,"Wxt536 Get Precipitation Data:"
...
static void Help(void)
{
	fprintf(stdout,"Wxt536 Get Precipitation Data:Help.\n");
	fprintf(stdout,"Wxt536 Get Precipitation Data queries the Vaisala Wxt536 and gets the current precipitation data.\n");
	fprintf(stdout,"wxt536_command_precipitation_data_get [-serial_device|-se <filename>][-d[evice_address] <character>]\n");
